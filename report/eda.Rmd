...
```{r}
## Key terms of Trump vs Obama
## Take a sub-corpus and clean it
speech.tok2 <- tokens(corpus_subset(
  Smartphone_reviews.cp,
...
speech.co <- fcm(Smartphone_reviews.tk, 
                 context = "document",
                 tri = FALSE)
index <- Smartphone_reviews.freq %>% 
  ...
  data.frame() %>% 
  select(feature)
speech.co <- speech.co[index$feature, index$feature]
speech.co[speech.co <= 2000] <- 0
speech.co[speech.co > 2000] <- 1
