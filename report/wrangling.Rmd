...
outputs <- detector(Samsung$Reviews, max_length = 512L, truncation=TRUE) # takes around 7 min
test <- data.frame(outputs)
x <- seq(from = 1, to = 5184, by = 2)
table_new <- data.frame(bind_cols(c(x = test[,x])))
samsung_lang <- table_new %>% 
  pivot_longer(cols = c(1:2592), names_to = "ID", values_to = "Language") %>% 
  select(-ID)
...
# Creating two empty data frames for table
table_new <-data.frame()
apple_lang <- data.frame()

# Detect the Languages of Apple reviews
outputs <- detector(Apple$Reviews, max_length = 512L, truncation=TRUE) # takes around 20 min
test <- data.frame(outputs)
x <- seq(from = 1, to = 20102, by = 2)
table_new <- data.frame(bind_cols(c(x = test[,x])))
apple_lang <- table_new %>% 
  pivot_longer(cols = c(1:10051), names_to = "ID", values_to = "Language") %>% 
  select(-ID)
...
# Get rid of Languages with low amount reviews (ja, hi, de, el, sw, ur, fr)
# English
samsung_en <- Samsung %>% 
  filter(Language == "en")
...
samsung_pt <- Samsung %>% 
  filter(Language == "pt")
samsung_pt <- samsung_pt[-c(1,12),] 
samsung_pt$Language <- str_replace(samsung_pt$Language, "pt", "es")
samsung_es <- Samsung %>% 
  filter(Language == "es") %>% 
  rbind(samsung_pt)
...
outputs <- translator(samsung_es$Reviews, max_length = 512L, truncation= TRUE) # takes around 14 min 
test <- data.frame(outputs)
x <- seq(from = 1, to = 332, by = 1)
table_new <- data.frame(bind_cols(c(x = test[,x])))
samsung_es_en <- table_new %>% 
  pivot_longer(cols = c(1:332), names_to = "ID", values_to = "Reviews") %>% 
  select(-ID)
...
# Translate Spanish to English
outputs <- translator(apple_es$Reviews, max_length = 512L, truncation= TRUE) # takes around 40 min 
test <- data.frame(outputs)
x <- seq(from = 1, to = 1799, by = 1)
table_new <- data.frame(bind_cols(c(x = test[,x])))
apple_es_en <- table_new %>% 
  pivot_longer(cols = c(1:1799), names_to = "ID", values_to = "Reviews") %>% 
  select(-ID)
